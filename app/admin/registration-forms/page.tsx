        ...
        actions={[
          {
            ...
            description: 
              "**Update Required:** To use Auto-Headers, you must update your script.\n\n1. Open your Google Sheet → Extensions → Apps Script.\n2. **Delete everything** and paste the code below.\n3. Deploy → **New Deployment** (Important!) → Type: Web App → Execute as: **Me** → Access: **Anyone**.\n4. Copy the `Web App URL` and paste it in the form settings.\n\n```javascript\nfunction doPost(e){var p=JSON.parse(e.postData.contents);var s=SpreadsheetApp.getActiveSpreadsheet().getActiveSheet();if(p.action=='setup_headers'){s.clear();s.appendRow(p.fields);s.getRange(1,1,1,p.fields.length).setFontWeight('bold').setBackground('#eee');return r({'result':'success'})}else{var h=s.getRange(1,1,1,s.getLastColumn()).getValues()[0];var row=[];var d=p.data||p;h.forEach(function(k){row.push(d[k]||'')});s.appendRow(row);return r({'result':'success'})}}function r(d){return ContentService.createTextOutput(JSON.stringify(d)).setMimeType(ContentService.MimeType.JSON)}```"
          }
        ]}
      />
